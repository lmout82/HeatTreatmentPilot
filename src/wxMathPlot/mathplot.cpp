...
void mpFXY::Plot(wxDC & dc, mpWindow & w)
{
    ...
    else
    {
		...
		{
			GetNextXY(x, y);
			wxCoord cx0 = (wxCoord)((x - w.GetPosX()) * w.GetScaleX());
			wxCoord cy0 = (wxCoord)((w.GetPosY() - y) * w.GetScaleY());
		...
		{
			wxPoint *points = new wxPoint[end_index+1];
			for(int index=0; index<end_index+1; index++)
			{
				GetNextXY(x, y);
				points[index].x = ((x - w.GetPosX()) * w.GetScaleX());
				points[index].y = ((w.GetPosY() - y) * w.GetScaleY());
			}

			dc.DrawSpline(end_index+1, points);
			delete[] points;
		}
    }
...
{ 
    SetName(name);
    SetFont( (wxFont&) *wxNORMAL_FONT); // changement taille police caracteres
...
{ 
    SetName(name);
    SetFont( (wxFont&) *wxNORMAL_FONT); // idem
    SetPen( (wxPen&) *wxGREY_PEN);
    m_flags = flags;
    m_ticks = ticks;
}
    ...
    : wxScrolledWindow( parent, id, pos, size, flag, wxT("mathplot") )
{
    ...
    m_popmenu.Append( mpID_CENTER,     _("Centrer"),      _(""));
    m_popmenu.Append( mpID_FIT,        _("Fit"),         _(""));
    m_popmenu.Append( mpID_ZOOM_IN,    _("Zoom in"),     _(""));
    m_popmenu.Append( mpID_ZOOM_OUT,   _("Zoom out"),    _(""));
    //m_popmenu.AppendCheckItem( mpID_LOCKASPECT, _("Lock aspect"), _("Lock horizontal and vertical zoom aspect."));
    m_popmenu.Append( mpID_HELP_MOUSE,   _("Montrer les commandes de la souris..."),    _(""));
...
{
    wxMessageBox(_("Commandes de la souris:\n \
        - Souris Roulette: Scroll Vertical \n \
        - Souris Roulette+SHIFT: Scroll Horizontal\n \
        - Souris Roulette+CTRL: Zoom in/out"),_("Aide wxMathPlot"),wxOK,this);
}
